            ...
            <div className="top-nav-container bg-light1 dark:bg-dark3 h-[80px] flex justify-between items-center px-5">
                ...
                <div className="search-settings-container flex items-center gap-5">
                    ...
                    <div className="flex gap-3">
                        <button className="btn-search bg-light3 dark:bg-dark1 text-light4 text-xl flex justify-center items-center w-10 h-10 rounded-lg xl:hidden" onClick={openSearch}>
                            <div className="icon relative top-1"><iconify-icon icon="mingcute:search-line"></iconify-icon></div>
                        </button>
                        <button className="btn-toggle-dark bg-light3 dark:bg-dark1 text-xl hidden xl:flex justify-center items-center w-10 h-10 rounded-lg" onClick={toggleDark}>
                            <div className={`icon relative top-1 ${dark? "hidden":"text-light4"}`}><iconify-icon icon="mingcute:moon-fill"></iconify-icon></div>
                            <div className={`icon relative top-1 ${dark? "text-light3":"hidden"}`}><iconify-icon icon="mingcute:sun-fill"></iconify-icon></div>
                        </button>
                        ...
                        <div className={`books-container overflow-y-scroll pb-3 ${tabCategory? "flex flex-col flex-grow-0":"hidden"}`}>
                            {
                                books.map((book, index) => {
                                    return <Book book_id={book.id} abvr_code={book.abvr_code} title={book.title} number_of_hadis={book.number_of_hadis} setActiveBook={setActiveBook}/>
                                })
                            }
                        </div>
                        <div className={`chapters-container overflow-y-scroll pb-3 ${tabCategory? "hidden":"flex flex-col flex-grow-0"}`}>
                            {
                                chapters.map((chapter, index) => {
                                    if(chapter.book_id === activeBook){
                                        return <Chapter chapter_id={chapter.chapter_id} title={chapter.title} hadis_range={chapter.hadis_range} setActiveChapter={setActiveChapter}/>
                                    }
                                })
                            }
                        </div>
                                    ...
                                    if(book.id === activeBook){
                                        return(
                                            <div className="breadcrumb-container bg-light2 dark:bg-dark3 text-light4 dark:text-light4 flex items-center px-5 py-3 rounded-t-lg">
                                                ...
                                                <div className="text text-xs">{book.book_name}</div>
                                                <div className="icon relative top-[2px]"><iconify-icon icon="iconamoon:arrow-right-2-light"></iconify-icon></div>
                                                {
                                                    chapters.map((chapter, _) => {
                                                        if(chapter.id === activeChapter){
                                                            return(
                                                                <div className="text text-xs">{chapter.id}</div>
                                                            )
                                                        }
                                                    })
                                                }
                                            ...
                                            <div className="flex justify-between items-center px-5 py-3 gap-3">
                                                <div className="left flex items-center gap-5">
                                                    ...
                                                    <div className="text bangla">
                                                        <div className="title text-lg text-dark4 dark:text-light3 font-medium">{book.title}</div>
                                                        <div className="subtitle text-sm text-light4">সর্বমোট হাদিস - {book.number_of_hadis}</div>
                                                    </div>
                                                </div>
                                                <div className="right arabic text-2xl font-medium dark:text-light3">{book.title_ar}</div>
                                            </div>
                                        )
                                    }
                                })
                                        ...
                                        <div className="chapter-container flex flex-col flex-grow-1 gap-3">
                                            ...
                                            </div>
                                            {
                                                sections.map((section, section_index) => {
                                                    if(section.chapter_id === activeChapter){
                                                        return(
                                                            <div className="section-container flex flex-col flex-grow-1 gap-3">
                                                                ...
                                                                </div>
                                                                {
                                                                    hadiths.map((hadith, hadith_index) => {
                                                                        // if(hadith.section_id === (section_index+1)){
                                                                        if(hadith.section_id === section.id){
                                                                            console.log("section ID: ",section.id, "hadith ID: ", hadith.hadith_id);
                                                                            return(
